...
# these models propagates only the mean value; no need for BlockEnsembles in this case, as the 
# parameter shouldn't be varying

abstract type AverageModel <: DeterministicNonlinearModel end

AverageModel(args...) = @abstractmethod

struct AverageODEModel <: AverageModel
  integrators::ODEIntegrator
end

AverageModel(integrator::ODEIntegrator) = AverageODEModel(integrator)
AverageModel(probl::ODEProblem,args...;kwargs...) = AverageModel(get_integrators(probl,args...;kwargs...))

function return_cache(a::AverageODEModel,d::Ensemble)
  y = similar_law(d)
  m = similar_mean(d)
  (y,m)
end

function evaluate!(cache,a::AverageODEModel,d::Ensemble)
  y,m = cache
  copyto!(a.integrator.u,mean(d))
  step!(a.integrator)
  copyto!(mean(y),a.integrator.u)
  @inbounds @views for i in axes(d.values,2)
    @. m = (d.values[:,i] - mean(d)) ./ mean(d)
    d.values[:,i] = mean(y) + mean(y) .* m
  end
  y
end

struct AverageTransientPDEModel <: AverageModel
  sol::GenericODESolution
end

AverageModel(sol::GenericODESolution) = AverageTransientPDEModel(sol)

function return_cache(a::AverageTransientPDEModel,d::Ensemble)
  t0 = a.sol.t0
  state0,odecache = ode_start(a.sol.odeslvr,a.sol.odeop,t0,a.sol.us0)
  statef = copy.(state0)
  uf = copy(first(a.sol.us0))
  c = ODECache(t0,statef,state0,uf,odecache)
  y = similar_law(d)
  m = similar_mean(d)
  (y,c,m)
end

function evaluate!(cache,a::AverageTransientPDEModel,d::Ensemble)
  y,c,m = cache
  @unpack r0,state0,statef,uf,odecache = c 
  to_state!(state0,mean(d),a.sol.odeslvr)
  cacheit = (r0,state0,statef,uf,odecache)
  (rf,uf),cacheitf = iterate(a.sol,cacheit)
  update!(c,cacheitf)
  copyto!(mean(y),uf)
