...
"""
    struct InflationFilter{A<:KalmanFilter,B<:InflationModel} <: KalmanFilter
...
Construct via `InflationFilter(filter; inflation=MultInflation())` or from raw
transition/observation/prior arguments (which automatically includes localisation).
"""
struct InflationFilter{A<:Filter,B<:InflationModel} <: Filter
  filter::A
  inflation::B
...
function InflationFilter(f::Filter,i::NLLInflation) 
  msg1 = "InflationFilter with NLLInflation is only implemented for linear  
  observation models. For nonlinear models, use MultInflation instead."
  msg2 = "InflationFilter with NLLInflation is not implemented for square-root 
...
get_prior(f::InflationFilter) = get_prior(f.filter)
get_observation_prior(f::InflationFilter) = get_observation_prior(f.filter)
get_transition_model(f::InflationFilter) = get_transition_model(f.filter)
get_observation_model(f::InflationFilter) = get_observation_model(f.filter)
get_noise(f::InflationFilter) = get_noise(f.filter)
get_observation_noise(f::InflationFilter) = get_observation_noise(f.filter)
get_cache(f::InflationFilter) = get_cache(f.filter)

get_inflation_parameter(f::InflationFilter) = get_parameter(f.inflation)
get_stashed_prior(f::InflationFilter) = get_stashed_prior(f.cache)
get_stashed_obs_prior(f::InflationFilter) = get_stashed_obs_prior(f.cache)
get_param_cache(f::InflationFilter) = get_param_cache(f.cache)

function transition!(posterior::SecondMoment,f::InflationFilter)
  transition!(posterior,f.filter)
end

function observation!(f::InflationFilter,posterior::SecondMoment)
  observation!(f.filter,posterior)
end

function innovation!(f::InflationFilter,z::InType)
  innovation!(f.filter,z)
end

function inflate_covariance!(posterior::SecondMoment,f::InflationFilter)
...
function inflate_covariance!(posterior::Ensemble,f::InflationFilter)
  ρ = get_inflation_parameter(f)
  obs_prior = get_observation_prior(f)
...
function inflate_covariance!(posterior::ConstrainedEnsemble,f::InflationFilter)
  inflate_covariance!(remove_constraint(posterior),f)
end

function kalman_gain!(f::InflationFilter,posterior::SecondMoment)
  inflate_covariance!(posterior,f)
  kalman_gain!(f.filter,posterior)
end

function update!(posterior::SecondMoment,f::InflationFilter,ỹ::InType)
  update!(posterior,f.filter,ỹ)
end

reset!(f::InflationFilter) = reset!(f.filter)

"""
    const NLLInflationKalmanFilter{A<:KalmanFilter} = InflationFilter{A,<:NLLInflation}
"""
const NLLInflationKalmanFilter{A<:Filter} = InflationFilter{A,<:NLLInflation}
