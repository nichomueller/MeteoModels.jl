...
# jacs = get_jacs(op)
# jac = jacs[1]
# uh = zero(X(get_params(μ),get_times(μ)))
# uht = TransientCellField(uh,(uh,)) 
# du = get_trial_fe_basis(Y)
# dv = get_fe_basis(Y)
# dc = jac(get_params(μ),get_times(μ),uht,du,dv)
# dcΩ = dc[Ω];
# cache = array_cache(dcΩ);
# cell = 80
# val = getindex!(cache,dcΩ,cell)
# _val = dcΩ[cell]
# val ≈ _val
...
execute(true_transition,ts)

# fesnaps, = solution_snapshots(ode_solver,op,true_μ,true_ic)